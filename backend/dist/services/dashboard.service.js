...
const getCutDate = (year, month) => new Date(Date.UTC(year, month, 1));
...
const getThresholdPeriodLabel = (cutDate) => {
    const year = cutDate.getUTCFullYear();
    const month = cutDate.getUTCMonth();
    if (month === 0)
...
const formatCutLabel = (cutDate) => {
    const month = cutDate.getUTCMonth() === 0 ? "Enero" : "Julio";
    return `${month} ${cutDate.getUTCFullYear()}`;
};
const getEstimatedCategory = (amount, categories) => {
    const sorted = [...categories].sort((a, b) => a.max_annual_income - b.max_annual_income);
    return (sorted.find((category) => amount <= category.max_annual_income) ??
        sorted[sorted.length - 1]);
};
const getAdjacentCategories = (categoryId, categories) => {
    const sorted = [...categories].sort((a, b) => a.max_annual_income - b.max_annual_income);
...
const getCategoriesForCut = async (cutDate) => {
    ...
    const latestAvailable = await db_1.prisma.category.findFirst({
        where: {
            effective_from: {
                lte: new Date(),
            },
...
const getAccumulatedForWindow = async (userId, from, to) => {
...
const buildCutSummary = async (userId, cutDate) => {
    ...
    const { previousCategory, nextCategory } = getAdjacentCategories(estimatedCategory.id, categories);
    return {
        ...
        remaining_to_next: nextCategory
            ...
            : null,
        percentage_used: formatMoney(estimatedCategory.max_annual_income > 0
            ? (accumulated / estimatedCategory.max_annual_income) * 100
            : 0),
    };
};
const buildProjectedCategories = (categories, estimatedIpc) => {
    return categories.map((category) => ({
        ...category,
        max_annual_income: formatMoney(category.max_annual_income * (1 + estimatedIpc / 100)),
    }));
};
const getMonthsRemainingForCut = (cutDate, now = new Date()) => {
...
const getMonthStart = (year, month) => new Date(Date.UTC(year, month, 1));
const getMonthEnd = (year, month) => new Date(Date.UTC(year, month + 1, 0, 23, 59, 59, 999));
const formatMonthLabel = (date) => `${MONTH_LABELS[date.getUTCMonth()]} ${date.getUTCFullYear()}`;
const getPreviousMonths = (count, now = new Date()) => {
    const months = [];
...
const getRolling12WindowForMonth = (monthDate) => {
...
const getMonthlyBilled = async (userId, monthDate) => {
    const year = monthDate.getUTCFullYear();
    const month = monthDate.getUTCMonth();
...
const buildLast6Months = async (userId, now = new Date()) => {
    ...
    for (const monthDate of months) {
        ...
        const accumulated12m = await getAccumulatedForWindow(userId, rollingWindow.from, rollingWindow.to);
        ...
        rows.push({
            ...
            category_limit: formatMoney(estimatedCategory.max_annual_income),
            margin: formatMoney(Math.max(estimatedCategory.max_annual_income - accumulated12m, 0)),
        });
    }
    return rows;
};
const getDashboardSummary = async (userId, estimatedIpc = 15) => {
    ...
    const partialCuts = [nextCutSummary, futureCutSummary].filter(Boolean);
    let projection = null;
    let projection_details = null;
    let progress = null;
    if (nextCutSummary) {
        const nextCutCategories = await getCategoriesForCut(nextCut);
        const nextEstimatedCategory = nextCutCategories.find((c) => c.code === nextCutSummary.category_code);
        ...
        const remainingToNext = nextAdjacent.nextCategory &&
        ...
        const projectedCategories = buildProjectedCategories(nextCutCategories, estimatedIpc);
        const projectedEstimatedCategory = getEstimatedCategory(nextCutSummary.accumulated, projectedCategories);
        const projectedAdjacent = getAdjacentCategories(projectedEstimatedCategory.id, projectedCategories);
        const projectedRemainingToNext = projectedAdjacent.nextCategory &&
            ...
            : 0;
        const allowedPerMonthWithoutUpdate = monthsRemaining > 0 ? remainingToNext / monthsRemaining : 0;
        const allowedPerMonthWithUpdate = monthsRemaining > 0 ? projectedRemainingToNext / monthsRemaining : 0;
        projection = {
            months_remaining: monthsRemaining,
            remaining_to_next: formatMoney(remainingToNext),
            estimated_ipc: estimatedIpc,
            allowed_per_month_without_update: formatMoney(Math.max(allowedPerMonthWithoutUpdate, 0)),
            allowed_per_month_with_update: formatMoney(Math.max(allowedPerMonthWithUpdate, 0)),
        };
        projection_details = {
        ...
        const progressPercent = progressLimit > 0 ? (progressUsed / progressLimit) * 100 : 0;
        progress = {
